...
const OptionLanguange = () => {
    ...
    return (
        <div className="relative bg-[1A1510] text-white rounded border border-gray-400 w-[200px] pl-8">
            ...
            <select onClick={handleSelect} className="w-full bg-[1A1510] py-1">
                <EachUtils of={LIST_LANGUANGE} render={(item, index) => (<option key={index} value={index.value}>{item.name}</option>)} />
            </select>
        </div>
    )
}
